...
export const BalanceCard: React.FC<BalanceCardProps> = ({ account, isActive, onClick, onSend, onAdd }) => {
  ...
  return (
    <motion.div
      ...
      whileTap={{ scale: 0.98 }}
      onClick={onClick}
      className={`sleek-card cursor-pointer transition-all relative overflow-hidden ${
    ...
    >
      {hasWarning && (
        <div className={`absolute top-0 right-0 px-3 py-1 text-[8px] font-black uppercase tracking-wider rounded-bl-xl shadow-sm ${
      ...
      <div className="mt-6 flex gap-3">
        <button 
          ...
          <ArrowUpRight size={14} /> {isDisabled ? 'Frozen' : 'Send'}
        </button>
        <button 
          onClick={(e) => {
            e.stopPropagation();
          ...
          <ArrowDownLeft size={14} /> {isDisabled ? 'Frozen' : 'Add'}
        </button>
      </div>
    </motion.div>
  );
