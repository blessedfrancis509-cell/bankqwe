            ...
            <div className="space-y-6">
              <div className="bg-slate-50 border border-slate-100 p-6 rounded-2xl space-y-4">
                <div className="space-y-2">
                  ...
                  >
                    {accounts.filter(a => a.status !== 'disabled').map(acc => (
                      <option key={acc.id} value={acc.id}>{acc.name} {acc.flagged ? '(Flagged)' : ''}</option>
                    ))}
                  </select>
                  {accounts.find(a => a.id === targetAccountId)?.status === 'disabled' && (
                    <p className="text-[10px] text-red-500 font-bold mt-1">This account is disabled. Select another.</p>
                  )}
                  {accounts.find(a => a.id === targetAccountId)?.depositRestricted && (
                    <p className="text-[10px] text-amber-500 font-bold mt-1">Deposits are restricted for this account.</p>
                  )}
                </div>
                <div className="space-y-2">
