...
export const AdminPanel = (props: { onMutation?: () => void }) => {
  ...
  const [transactions, setTransactions] = useState<{ id: string, accountId: string, amount: number, description: string, category: string, status: string, type: string, date: string }[]>([]);
  ...
  const [editTxCat, setEditTxCat] = useState('Other');
  const [editTxDate, setEditTxDate] = useState('');
  const [editTxStatus, setEditTxStatus] = useState<'completed' | 'pending' | 'rejected'>('completed');
    ...
    try {
      ...
      if (accs.length > 0) {
        ...
        if (currentSelected) {
          ...
          setIsFlagged(currentSelected.flagged || false);
          setAccountCreatedAt(currentSelected.createdAt ? currentSelected.createdAt.split('T')[0] : '');
  ...
  const handleAccountChange = (id: string, switchTab = false) => {
    ...
    if (acc) {
      ...
      setIsFlagged(acc.flagged || false);
      setAccountCreatedAt(acc.createdAt ? acc.createdAt.split('T')[0] : '');
    ...
    try {
      const res = await fetch(`/api/admin/accounts/${selectedAccId}`, {
        ...
        body: JSON.stringify({ 
          ...
          flagged: isFlagged,
          createdAt: accountCreatedAt ? new Date(accountCreatedAt).toISOString() : undefined
        })
      });
      if (!res.ok) {
        const err = await res.json();
  ...
  const handleEditTx = (tx: typeof transactions[0]) => {
    setEditingTxId(tx.id);
    setEditTxDesc(tx.description);
    setEditTxAmount(String(tx.amount));
    setEditTxCat(tx.category);
    setEditTxDate(tx.date ? tx.date.split('T')[0] : '');
    setEditTxStatus(tx.status as any);
  };
  ...
  const handleSaveEditTx = async () => {
    ...
    try {
      const res = await fetch(`/api/admin/transactions/${editingTxId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      showStatus('Failed to edit transaction', 'error');
    }
  };
    ...
    <div className="space-y-8 pb-20">
              ...
              <div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1 block">Account Creation Date</label>
                      <input 
                        type="date"
                        value={accountCreatedAt}
                        onChange={(e) => setAccountCreatedAt(e.target.value)}
                        className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl font-bold text-slate-850 text-xs text-slate-800"
                      />
                    </div>
                  </div>
            ...
            <div className="sleek-card bg-slate-905 text-white bg-slate-900 border-slate-800 animate-in fade-in duration-300">
              ...
              <div className="space-y-6">
                <div className="space-y-3">
                  ...
                  >
                     <div className="flex items-center gap-3">
                       <Flag size={20} className={isFlagged ? 'text-red-400' : 'text-slate-400'} />
                       <div>
                          <p className="text-sm font-bold">Account Flagged</p>
                          <p className="text-[10px] text-slate-450 uppercase font-black">
                            {isFlagged ? 'Flagged for Review' : 'Clean (No Flags)'}
                          </p>
                       </div>
                     </div>
                     <span className={`px-2 py-1 text-[9px] font-black uppercase rounded ${
              ...
              <div className="sleek-card border-dashed border-2 bg-white animate-in fade-in duration-300">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                   ...
                   <div>
                     <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1 block">Profile Photo URL</label>
                     <div className="flex gap-3 items-center">
                       {newUserPhoto && (
                         <img src={newUserPhoto} alt="Preview" className="w-10 h-10 rounded-full object-cover border border-slate-200 shrink-0" />
                       )}
                       <input 
                         type="text"
      ...
      {activeView === 'transactions' && (
        <div className="space-y-6">
          ...
          {/* Transaction List with Edit */}
          <div className="sleek-card">
            <h3 className="font-bold text-slate-800 mb-6 flex items-center gap-2">
              <Database size={20} className="text-indigo-600" /> Recent Transactions
            </h3>
            <div className="space-y-4">
              {transactions.slice(0, 20).map(tx => {
                ...
                return (
                  <div key={tx.id} className={`border rounded-xl p-4 ${isEditing ? 'border-indigo-300 bg-indigo-50/30' : 'border-slate-200 bg-white'}`}>
                    {isEditing ? (
                      <div className="space-y-3">
                        <div className="flex justify-between items-start mb-2">
                          <span className="text-[10px] font-bold text-indigo-600 uppercase tracking-widest">Editing: {tx.id}</span>
                          <button onClick={handleCancelEditTx} className="text-[10px] text-slate-400 hover:text-slate-600 font-bold uppercase">Cancel</button>
                        </div>
                        <div className="grid grid-cols-2 gap-3">
                          <div className="col-span-2">
                            <label className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-1 block">Description</label>
                            <input type="text" value={editTxDesc} onChange={(e) => setEditTxDesc(e.target.value)} className="w-full p-2 bg-white border border-slate-200 rounded-lg text-xs font-bold text-slate-800" />
                          </div>
                          <div>
                            <label className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-1 block">Amount</label>
                            <input type="number" value={editTxAmount} onChange={(e) => setEditTxAmount(e.target.value)} className="w-full p-2 bg-white border border-slate-200 rounded-lg text-xs font-bold text-slate-800" />
                          </div>
                          <div>
                            <label className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-1 block">Category</label>
                            <select value={editTxCat} onChange={(e) => setEditTxCat(e.target.value)} className="w-full p-2 bg-white border border-slate-200 rounded-lg text-xs font-bold text-slate-800">
                              <option>Income</option>
                              <option>Food</option>
                              <option>Entertainment</option>
                              <option>Lifestyle</option>
                          ...
                          <div>
                            ...
                            <select value={editTxStatus} onChange={(e) => setEditTxStatus(e.target.value as any)} className="w-full p-2 bg-white border border-slate-200 rounded-lg text-xs font-bold text-slate-800">
                          ...
                          <div>
                            <label className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-1 block">Date</label>
                            <input type="date" value={editTxDate} onChange={(e) => setEditTxDate(e.target.value)} className="w-full p-2 bg-white border border-slate-200 rounded-lg text-xs font-bold text-slate-800" />
                          </div>
                        </div>
                        <button onClick={handleSaveEditTx} className="w-full py-2 bg-indigo-600 text-white rounded-lg text-[10px] font-bold uppercase tracking-widest hover:bg-indigo-700 transition-all">
                          Save Changes
                        </button>
                      </div>
                    ) : (
                      <div className="flex items-center justify-between gap-4">
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-[10px] text-slate-400 font-medium">
                            {acc?.name || 'Unknown Account'} &middot; {new Date(tx.date).toLocaleDateString()}
                          </p>
                        </div>
                        ...
                        <button onClick={() => handleEditTx(tx)} className="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-all">
                          <Pencil size={14} />
                        </button>
                      </div>
                    )}
              ...
              {transactions.length === 0 && (
                <p className="text-center py-8 text-slate-400 text-xs italic">No transactions recorded yet.</p>
              )}
            </div>
          </div>
        </div>
         ...
         <div className="divide-y divide-slate-200">
           {accounts.map(acc => (
             <div key={acc.id} className="py-4 flex flex-col sm:flex-row justify-between sm:items-center gap-3">
                   <div className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-full bg-indigo-100 border border-indigo-200 overflow-hidden flex items-center justify-center font-bold text-xs text-indigo-600 shadow-sm shrink-0">
                       {(() => {
                         const owner = usersList.find(u => u.uid === acc.userId);
                    ...
                    <p className="text-sm font-bold text-slate-800 leading-none mb-1">{acc.name}</p>
                        <p className="text-[10px] text-slate-400 font-mono">UID: {acc.id} | Limit: ${acc.creditLimit?.toLocaleString()} | Created: {acc.createdAt ? new Date(acc.createdAt).toLocaleDateString() : 'N/A'}</p>
                    </div>
                 </div>
                <div className="flex items-center justify-between sm:text-right gap-6">
                   <div className="sm:text-right">
                      <p className="text-sm font-bold text-slate-800">${acc.balance.toLocaleString()}</p>
                       <p className={`text-[10px] font-bold uppercase tracking-tight ${acc.status === 'disabled' ? 'text-red-500' : acc.flagged ? 'text-red-500' : 'text-emerald-600'}`}>
